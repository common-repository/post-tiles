...
function register_post_tiles_options(){
	...
	register_setting( 'tc-plugin-settings', 'posttile-transparency');
	...
	foreach($categories as $category) {
		...
		register_setting('tc-plugin-settings', "cat_bg_".$cat_slug);
		register_setting('tc-plugin-settings', "cat_tx_".$cat_slug);
	}
}
...
function tc_post_tiles_options() {
	...
	wp_enqueue_style( 'post-tiles', $plugin_url.'/post-tiles.css', false, filemtime( plugin_dir_path( __FILE__ ).'/post-tiles.css' ), 'all' );
	...
	$posttile_transparency = get_option('posttile-transparency');
	if(empty($posttile_transparency)){
		$posttile_transparency = "100%";
	}
	...
	foreach($categories as $category) {
		$cat_bg_var = "cat_bg_".$category->name;
		$cat_tx_var = "cat_tx_".$category->name;
		$cat_slug = $category->slug;
		// Make lowercase
		$cat_bg_var = strtolower($cat_bg_var);
		$cat_bg_var = str_replace(" ", "_", $cat_bg_var);
		$cat_tx_var = strtolower($cat_tx_var);
		$cat_tx_var = str_replace(" ", "_", $cat_tx_var);
		$cat = $category->name;
		$id = $category->cat_ID;
		// Retrieves option value
		$cat_bg_var_value = get_option("cat_bg_".$cat_slug);
		$cat_tx_var_value = get_option("cat_tx_".$cat_slug);
		// Checks the value to see if it's empty. If it is use default.
		if (empty($cat_bg_var_value)){
			$cat_bg_var_value = "#afe2f8";
		}
		if (empty($cat_tx_var_value)){
			$cat_tx_var_value = "#ffffff";
		}

		// Echo out each list Category Name > Id > Input
		echo "<tr><td><strong>".$cat." </strong> <em>(".$id.")</em></td>
		<td><input title='Category background color' type='text' class='colorwell background' name='cat_bg_".$cat_slug."' value='".$cat_bg_var_value."' /></td><td><input title='Category text color'  type='text' class='colorwell text' name='cat_tx_".$cat_slug."' value='".$cat_tx_var_value."' />
		</td></tr>";
	}
	...
	<li class='category-key-list-item'>
	  <div class='category-key-radio'>
		  <select name="posttile-transparency" id="posttile-transparency">
		    <?php for ($i=0;$i<96;$i+=5): ?>
		    <option value="<?php echo $i/100 ?>" <?php if($posttile_transparency == $i/100) echo 'selected="selected"'; ?> ><?php echo $i ?>%</option>
	        <?php endfor; ?>
				<option value="0" <?php if($posttile_transparency == 0) echo 'selected="selected"'; ?> >100%</option>
		  </select>
	  </div>
	  <div class='category-key-admin'>
		  <strong>Tile Trasnparency</strong>
		  <br/>
		  <small><em>Pick from one of the transparency levels.</em></small>
	...
	<li class='category-key-list-item'>
	  <div class='category-key-radio'>
		  <select name="animation-style" id="animation-style">static-over
			<option value="static-over" <?php if($animation_style == 'static-over') echo 'selected="selected"'; ?>>Static Overlay (no jQuery required)</option>
			<option value="static-hide" <?php if($animation_style == 'static-hide') echo 'selected="selected"'; ?>>Static No Overlay (no jQuery required)</option>
	  ...
	  <div class='category-key-admin'>
		  ...
		  <!-- An alert that this feature requires jQuery be turned on -->
		  <?php if($show_key_jquery == 'false') echo '<br/><small class="attention"><em><strong>ATTENTION:</strong> This feature may require jQuery be turned on</em></small>'; ?>
	  </div>
	</li>
	...
	<div class="post-slides">
		<h3>Post Slides</h3>
		<p>If you like Post Tiles, also check out Post Slides. A new responsive and touch friendly post carousel plugin. <a href="https://wordpress.org/plugins/post-slides/" target="_blank" class="post-slides-button">Learn More</a></p>
		<a href="https://wordpress.org/plugins/post-slides/" target="_blank" class="image-anchor"><img src="http://www.postslides.com/wp-content/uploads/2014/11/slides-compressed.gif" alt="post-slides"></a>
		<a href="https://wordpress.org/plugins/post-slides/" target="_blank" class="image-anchor"><img src="http://www.postslides.com/wp-content/uploads/2014/11/slide-hover.gif" alt="post-slides"></a>
	</div>
...
function tc_post_tiles($atts,$content=null)
{
	...
	// EXAMPLE USAGE: [post-tiles categories="1,2,3,4" posts="8"]
	   ...
	   extract(shortcode_atts(array(
		...
		} else if (get_option('excerpt-length') != false ) {
			$excerpt_length = get_option('excerpt-length');
		}	else {
			$excerpt_length = '12';
		}
	   ...
	   // Retrieves The Tile Trasnparency
	   $posttile_transparency = get_option('posttile-transparency');
	   if(empty($posttile_transparency)){
	   	  $posttile_transparency = "100";
	   }
	   ...
	   if (have_posts()) : while (have_posts()) : the_post();
	      ...
	      // Define the Settings for recording
	      $cat_bg_var = "cat_bg_".$category_slug;
	      $cat_tx_var = "cat_tx_".$category_slug;
	      // Make lowercase
	      //$cat_var_value = strtolower($cat_var);
	      //$cat_var = str_replace(" ", "_", $cat_var);
	      // Retrieve the option "hexadecimal value" for this category
	      $cat_bg_var_value = get_option($cat_bg_var);
	      $cat_tx_var_value = get_option($cat_tx_var);
	      // Checks the value to see if it's empty. If it is use default.
	      if (empty($cat_bg_var_value)){
	      	$cat_bg_var_value = "#afe2f8";
	      }
	      ...
	      $featured_border = "border-bottom: 3px solid ".$cat_bg_var_value.";border-right: 2px solid ".$cat_bg_var_value.";";
		  ...
		  if ($featured_images_key == "true") {
		      if ( has_post_thumbnail()) {
			      ...
			      $url = $thumb['0'];
			      $featured_style = "style='background: url(".$url.") no-repeat center center; background-size: cover;' class='featured-image ".$animation_style." ".$cat_bg_var."'";
		      } else {
				  $featured_style = "class='".$cat_bg_var."'";
			  }
	      }
	      // Each output creates the li > link > title > exerpt
	      $rgbColor = hex2rgb($cat_bg_var_value);
	      $rgba_value = "rgba(".$rgbColor[0].",".$rgbColor[1].",".$rgbColor[2].",".$posttile_transparency.")";
	      $output .= "<li ".$featured_style." class='".$cat_bg_var."' id='".$theID."'><a href='$temp_link' class='".$cat_bg_var."' style='background-color: $rgba_value; color: $cat_tx_var_value; $featured_border'><h3 style='color: $cat_tx_var_value;'>$temp_title </h3>$temp_excerpt</a></li>\n";
	   ...
	   if ($show_key == "true"){
			...
			foreach($categories as $category) {
					...
					$cat_bg_var = $category->name;
					// Sets the slug
					$cat_slug = $category->slug;
					// Cleans up category names that have spaces
					$cat_bg_var_key = "cat_bg_".$cat_slug;
					// Get's the category options which are the hexadecimal colors
					$cat_bg_var_key_val = get_option($cat_bg_var_key);
					// loops through the each category and prints them
					$key_items .= "<a href='#' class='key' id='".$cat_bg_var_key."' style='background-color:".$cat_bg_var_key_val.";'>".$cat_bg_var."</a>\n";
			}
...
// Adds tranformations from Hex to RGB color
// Example: hex2rgb("#cc0")
